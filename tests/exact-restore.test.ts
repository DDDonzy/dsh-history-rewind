...
import { WORKSPACE_DEFAULT_EXCLUDES } from '../src/constants.ts'
...
test('materializeTreeExact restores exactly: extra files removed, excluded dirs kept, empty dirs pruned', async () => {
  ...
  // --- Snapshot A: one file at the workspace root. ---
  ...
  // --- Exact restore back to A. ---
  const excludes = WORKSPACE_DEFAULT_EXCLUDES
  ...
  assert.equal(restored, 1, 'snapshot A had exactly one file')
  ...
  assert.deepEqual(rootEntries, ['game.html', 'node_modules'])
