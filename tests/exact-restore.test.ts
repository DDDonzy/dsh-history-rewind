...
test('materializeTreeExact restores exactly: extra files removed, excluded dirs kept, empty dirs pruned', async () => {
  ...
  // --- Snapshot A: one file at the workspace root, plus a .gitignore that excludes node_modules. ---
  ...
  // --- Exact restore back to A, using the workspace's OWN .gitignore. ---
  const excludes = await readExcludes(cwd)
  assert.ok(excludes.includes('node_modules'), '.gitignore excludes node_modules')
