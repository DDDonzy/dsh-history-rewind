...
9.5 成功边界之后：清理**本次**在 `backups/` 留下的副本（步骤 3 的会话副本 +
    步骤 5 的工作区副本）。作用域严格限定在本次调用记录的路径，历史备份一律
    不动；会话副本仅在「替换 + resume 均成功」后删除，工作区副本仅在其恢复
    确实成功后删除（恢复失败不会中断回退，但那种半写状态正是备份的用途，
    必须留存）。清理是 best-effort：删不掉不会把成功的回退改判为失败，
    而是把残留路径回报给调用方（`backup` + `backupCleanupFailed`）。
...
**工作区回退链路**：解析目标 commit 的 `ws=` 配对 → 备份当前工作区 → `materializeTreeExact` 精确恢复（`read-tree` + `checkout-index -a -f` 写回目标文件；随后按**与快照 walk 完全相同的排除规则**（即恢复时刻工作区自己的 `.gitignore`）遍历工作区，删除「目标树中不存在的 in-scope 文件」并修剪空目录）。结果与快照**逐文件一致，不多不少**。`.git` 与 `.gitignore` 命中的排除目录从不进入遍历，绝不误删；跳转前已整目录备份到 `backups/`，删除可恢复——该副本在恢复确认成功后即被清理，恢复失败则保留。
...
- **删除范围**：回退后目标之后的事件不在会话记录与官方日志文件中；git 影子历史保留（撤销/审计的主路径就是重新跳回去）；`backups/` 只保留失败或半成功那几次的副本，成功回退会清理自己的副本；安全擦除走 `/purge`（不可恢复）；
- **备份不是撤销主路径**：`backups/` 兜的是「尚未进入影子仓库的状态」——回退链中途失败的半写状态，以及回退前未被任何快照记录过的改动。成功回退后自动清理意味着这类未快照改动**不再能从 `backups/` 找回**；已进入影子仓库的版本不受影响，随时可再跳回。清理只省磁盘，不省回退前的复制耗时；
...
- **工作区精确恢复**：恢复后工作区与快照逐文件一致（多余的 in-scope 文件会被删除、空目录会被修剪）；`.git` 与工作区 `.gitignore` 命中的目录不受影响，且跳转前整目录已备份（成功后自动清理，失败保留）；
