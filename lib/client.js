...
function GitignoreTemplateCard() {
	...
	return (0, react.createElement)("div", { style: {
		...
		fontWeight: 600,
		fontSize: 13
	} }, "默认 .gitignore 模板"), (0, react.createElement)("span", { style: {
		fontSize: 12,
		color: "var(--dsw-alias-label-secondary, #999)",
		lineHeight: 1.5
	} }, "仅在一个工作区第一次被快照、且该目录下还没有 .gitignore 文件时，会用这份内容自动创建一个。已存在的 .gitignore（无论是本来就有的，还是之前被这份模板创建过之后手动改过的）永远不会被覆盖或合并——快照的排除规则完全来自目标工作区自己的 .gitignore。"), (0, react.createElement)("textarea", {
		value: text,
		disabled: !loaded,
	...
	}, saving ? "保存中…" : "保存"), notice.length > 0 ? (0, react.createElement)("span", { style: {
		fontSize: 12,
		color: "var(--dsw-alias-label-secondary, #999)"
	} }, notice) : null));
}
...
function HistoryRewindSettingsPage() {
	return (0, react.createElement)("div", { style: {
		...
		lineHeight: 1.5,
		maxWidth: 520
	} }, "本插件用 git 影子仓库实现会话快照与回退，因此依赖 Git。快照时只按目标工作区自己的 .gitignore 排除文件。"), (0, react.createElement)(GitPluginCard), (0, react.createElement)(GitignoreTemplateCard));
}
