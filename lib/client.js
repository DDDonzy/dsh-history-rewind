window.__ModuleLoader__.load({ id: "@deepseek-ai/dsh-history", factory: (require) => {
"use strict";
...
async function rebindView(sessions, sessionId) {
	...
	if (record === void 0 || dropScope === void 0 || clearSelection === void 0 || openSession === void 0) {
		setTimeout(() => {
...
function FileChips(props) {
	...
	return (0, react.createElement)("div", { className: FILE_LIST }, files.slice(0, MAX_FILES).map((file) => (0, react.createElement)("code", {
		className: FILE_CHIP,
		title: file,
		key: file
...
function RowContentNode(props) {
	...
	if (kind === "turn-start") {
		...
		return (0, react.createElement)("div", { className: ROW_CONTENT }, (0, react.createElement)("div", { className: ROW_MAIN }, (0, react.createElement)("div", { className: `${SINGLE_LINE} ${FILE_LIST}` }, (0, react.createElement)("span", { className: "dsh-badge dsh-badge-turn-start" }, "BASELINE"), shown.map((file) => (0, react.createElement)("code", {
			className: FILE_CHIP,
			title: file,
			key: file
		}, file)), files.length > MAX_FILES ? (0, react.createElement)("code", {
			className: FILE_CHIP,
			style: { color: "var(--dsw-alias-label-secondary, #999)" }
		}, `… +${files.length - MAX_FILES}`) : null)), sideElements);
	}
	...
	return (0, react.createElement)("div", { className: ROW_CONTENT }, (0, react.createElement)("div", { className: ROW_MAIN }, (0, react.createElement)("div", { className: MSG_LIST }, meta?.userMessage ? (0, react.createElement)("div", { className: MSG_ITEM }, (0, react.createElement)("span", { className: `${MSG_ROLE} ${MSG_ROLE_USER}` }, "USER"), (0, react.createElement)("span", {
		className: MSG_TEXT,
	...
	}, meta?.asstMessage ?? meta?.message ?? "—"))), (0, react.createElement)(FileChips, { files })), sideElements);
}
...
function GraphCell(props) {
	...
	const nodeColor = onPath ? HOVER : hovering ? grey : onHead(row.lane) ? colors[row.lane % colors.length] : grey;
...
function HistoryPanel(props) {
	...
	*  layout effect below consumes it once the new slice is painted. */
	...
	const doRewind = async (withWorkspace) => {
		...
		setBusy(false);
		if (!result.ok) {
			setNotice(`${result.reason ?? "failed"}${result.error !== void 0 ? `：${result.error}` : ""}`);
			return;
		}
		setNotice(`已回退 ✓ ${result.target !== void 0 ? `目标 ${result.target.slice(0, 7)}` : ""}（继续对话将开辟新分支）${result.workspaceRestored === true ? " · 工作区已恢复" : ""}${result.noWorkspaceSnapshot === true ? " · 无配对工作区快照" : ""}${result.compositionWarning !== void 0 ? ` · ⚠ ${result.compositionWarning}` : ""}`);
		await load();
	...
	if (rows.length === 0) return (0, react.createElement)("div", {
		...
		lineHeight: 1.5,
		fontSize: 12
	} }, "当前会话尚未产生快照。发送消息后，TURN 开始与结束将自动记录。"), (0, react.createElement)("button", {
		className: `${BUTTON} primary`,
		disabled: busy,
		onClick: () => void doSnapshot()
	}, "立即快照"));
	...
	return (0, react.createElement)(
		...
		selected !== null ? (0, react.createElement)("div", {
...
function apply(ctx) {
	...
	const HistoryPanelShell = () => {
		...
		if (!open || current === void 0) return null;
		return (0, react.createElement)("div", {
			className: MODAL_BACKDROP,
