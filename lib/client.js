...
async function fetchTimeline(sessionId) {
	...
	if (data === null) return {
		ok: false,
		reason: "transport",
		error: "网络请求失败或插件服务无响应 (Transport Error)"
	};
...
const DIALOG_BODY = "dsh-history-dialog-body";
...
function HistoryPanel(props) {
	...
	*  immediately (the graph stays on HEAD unless the user wants to switch). */
	...
	const load = async () => {
		...
		if (result.ok && result.rows !== void 0) {
			...
			const errText = result.error ? `${result.error}${errDetail}` : result.reason === "git-unavailable" ? "Git 环境不可用：未检测到系统 Git 安装，无法读取快照。" : result.reason === "transport" ? "网络传输异常：无法连接至插件服务或请求超时。" : result.reason ? `错误代码: ${result.reason}${errDetail}` : "时间线数据读取异常（未初始化或数据异常）";
	...
	*  timestamp are treated as ours, not the user's. */
	...
	if (rows === null) return (0, react.createElement)("div", {
	...
	}, (0, react.createElement)("div", { className: DIALOG_HEAD }, (0, react.createElement)("h2", { className: DIALOG_TITLE }, "读取会话历史失败"), (0, react.createElement)("button", {
	...
	}, "读取当前会话快照时间线时发生异常："), (0, react.createElement)("pre", { style: {
		margin: 0,
		padding: "10px 12px",
		borderRadius: 8,
		...
		wordBreak: "break-all",
		userSelect: "text"
	} }, loadError ?? "未知错误（未初始化或数据异常）")), (0, react.createElement)("div", { className: DIALOG_FOOT }, (0, react.createElement)("button", {
	...
	if (rows.length === 0) return (0, react.createElement)("div", {
		...
		onClick: (event) => event.stopPropagation()
	}, (0, react.createElement)("div", { className: DIALOG_HEAD }, (0, react.createElement)("h2", { className: DIALOG_TITLE }, "暂无快照"), (0, react.createElement)("button", {
		className: DIALOG_CLOSE,
		title: "关闭",
		onClick: () => onRewound()
	}, "✕")), (0, react.createElement)("div", {
	...
	}, "当前会话尚未产生快照记录。发送消息后，TURN 开始与结束将自动为您记录快照。")), (0, react.createElement)("div", { className: DIALOG_FOOT }, (0, react.createElement)("button", {
		className: `${BUTTON} primary`,
		onClick: () => onRewound()
	}, "确定"))));
...
function apply(ctx) {
	...
	if (typeof window !== "undefined") {
		window.__DSH_HISTORY_REWIND_OPEN__ = (val = true) => setOpen(val);
	...
	*  (it closes immediately on confirm); delivered to the next panel mount. */
	...
	const HistoryPanelShell = () => {
		...
		const sessionId = current ?? lastId.current ?? (snap.ids && snap.ids.length > 0 ? snap.ids[0] : void 0);
		...
		*  is centered on the conversation center. */
		...
		const panel = open ? (0, react.createElement)(
			"div",
			{
				className: MODAL_BACKDROP,
			...
			(0, react.createElement)("div", {
				...
				style: {
					...
					...sessionBounds !== null ? {
						...
						transform: activeLanes > 0 ? `translateX(calc(-50% - ${(graphWidth(activeLanes) + 12) / 2}px))` : "translateX(-50%)",
						width: `min(860px, ${Math.max(300, sessionBounds.width - 48)}px)`
					} : {},
					...dialogOpen ? { display: "none" } : {}
				},
				onClick: (event) => event.stopPropagation()
			}, sessionId !== void 0 ? (0, react.createElement)(HistoryPanel, {
				sessionId,
				...
				onClick: (event) => event.stopPropagation()
			}, (0, react.createElement)("div", { className: DIALOG_HEAD }, (0, react.createElement)("h2", { className: DIALOG_TITLE }, "提示"), (0, react.createElement)("button", {
				className: DIALOG_CLOSE,
				title: "关闭",
				onClick: () => setOpen(false)
			}, "✕")), (0, react.createElement)("div", {
			...
			}, "未检测到活跃会话。请先在左侧选择或创建一个会话。")), (0, react.createElement)("div", { className: DIALOG_FOOT }, (0, react.createElement)("button", {
				className: `${BUTTON} primary`,
				onClick: () => setOpen(false)
			}, "确定")))))
) : null;
