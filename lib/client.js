...
function HistoryStateCard(props) {
	...
	const isBusy = actionBusy !== null && actionBusy !== void 0;
	...
	if (type === "loading") {
		...
		}, actionBusy === "reload" ? "正在检查…" : "重新检查"), (0, react.createElement)("button", {
			type: "button",
			className: `${BUTTON} primary`,
			disabled: isBusy,
		...
		}, actionBusy === "reload" ? "正在加载…" : "重新加载"), (0, react.createElement)("button", {
			type: "button",
			className: `${BUTTON} primary`,
			disabled: isBusy,
			onClick: onSnapshot,
...
function HistoryPanel(props) {
	...
	const [actionBusy, setActionBusy] = (0, react.useState)(null);
	...
	const doInstallGit = async () => {
		...
		try {
			...
			if (result.ok && result.installed === true) {
				setNotice("Git 安装成功 ✓");
				await load();
			} else setNotice(`Git 安装失败：${result.message ?? result.detail ?? "unknown"}`);
		} finally {
			setActionBusy(null);
		}
	};
