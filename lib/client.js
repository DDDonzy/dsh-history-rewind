...
function CacheCard() {
	...
	const doClear = async (scope) => {
		...
		setClearing(false);
		setDialogOpen(false);
		setNotice(result.ok ? `已清空 ✓ 释放 ${formatBytes(result.freedBytes ?? 0)}` : `清空未完成：${result.failed ?? 0} 项无法删除${result.reason !== void 0 ? `（${result.reason}）` : ""}`);
		await load();
	};
	...
	return (0, react.createElement)(
		...
		}), (0, react.createElement)("span", { style: labelStyle }, "GB"), (0, react.createElement)("button", {
			type: "button",
			disabled: saving || !loaded,
		...
		(0, react.createElement)("div", { style: {
			...
			...labelStyle,
			fontSize: 11
		} }, `会话 ${formatBytes(usage.sessionBytes)} · 工作区 ${formatBytes(usage.workspaceBytes)} · 备份 ${formatBytes(usage.backupsBytes)}`) : null), (0, react.createElement)("button", {
			type: "button",
		...
		notice.length > 0 ? (0, react.createElement)("span", { style: labelStyle }, notice) : null,
		...
		dialogOpen ? (0, react.createElement)("div", {
			style: {
				position: "fixed",
				inset: 0,
				zIndex: 10001,
			...
			fontWeight: 600,
			color: "var(--dsw-alias-label-primary, #e6e6e6)"
		} }, "清空快照缓存"), (0, react.createElement)("div", { style: {
			...labelStyle,
			lineHeight: 1.6
		} }, "选择要清空的范围。此操作会删除对应的影子仓库历史，", (0, react.createElement)("strong", { style: { color: "#e5534b" } }, "不可恢复"), "，清空后这些版本无法再回退。你的项目代码本身不受影响。"), (0, react.createElement)("div", { style: {
			display: "flex",
			flexDirection: "column",
			gap: 8
		} }, ...[
		...
		].map(([scope, title, desc]) => (0, react.createElement)("button", {
			key: scope,
			type: "button",
			disabled: clearing,
			...
			...labelStyle,
			fontSize: 11
		} }, desc)))), (0, react.createElement)("div", { style: {
			display: "flex",
			justifyContent: "flex-end"
...
function HistoryRewindSettingsPage() {
	return (0, react.createElement)("div", { style: {
		...
		maxWidth: 520,
		flex: "none"
	} }, "本插件用 git 影子仓库实现会话快照与回退，因此依赖 Git。"), (0, react.createElement)(GitPluginCard), (0, react.createElement)(GitignoreTemplateCard), (0, react.createElement)(CacheCard));
}
