...
import { deflateRawSync, inflateRawSync, zstdCompressSync, zstdDecompressSync } from "node:zlib";
...
*/
function argvCommitTree(repo, treeSha, parentSha) {
	const argv = [
		...base(repo),
		"commit-tree",
		treeSha,
...
async function readConfig(root) {
	try {
		...
		return {
			...
			cacheCapacityGb: typeof parsed.cacheCapacityGb === "number" && Number.isFinite(parsed.cacheCapacityGb) && parsed.cacheCapacityGb > 0 ? parsed.cacheCapacityGb : HISTORY_REWIND_DEFAULTS.cacheCapacityGb
		};
	} catch {
...
async function storedSessionCommit(repoDir) {
	const refs = new Map();
	const headsDir = join(repoDir, "refs", "heads");
	const looseNames = await readdir(headsDir).catch(() => []);
	...
	for (const [name, sha] of refs) {
		...
		const timestamp = Number(name.slice(5));
		if (Number.isFinite(timestamp) && (latestRoad === void 0 || timestamp > latestRoad.timestamp)) latestRoad = {
			timestamp,
			sha
		};
	}
...
async function storedSessionPresentation(root, sessionId) {
	...
	try {
		...
		const title = typeof rawTitle === "string" && rawTitle.trim().length > 0 ? rawTitle.trim() : void 0;
		const workspace = typeof rawCwd === "string" && rawCwd.length > 0 ? basename(rawCwd) : void 0;
		return {
			...title === void 0 ? {} : { title },
			...workspace === void 0 || workspace.length === 0 ? {} : { workspace }
		};
	} catch {
		return {};
	}
...
async function measureStoredSession(root, sessionId, withUsage) {
	...
	let sBackupBytes = 0;
	let wsBackupBytes = 0;
	if (withUsage) [sessionBytes, workspaceBytes, sBackupBytes, wsBackupBytes] = await Promise.all([
		dirSize(sRepo),
		dirSize(wsRepo),
		dirSize(sBackup),
		dirSize(wsBackup)
...
*/
async function listStoredSessions(root, withUsage = true) {
	const ids = new Set();
	const [reposEntries, wsEntries, backupEntries] = await Promise.all([
		readdir(join(root, REPOS_DIRNAME)).catch(() => []),
		readdir(join(root, REPOS_WS_DIRNAME)).catch(() => []),
		readdir(join(root, BACKUPS_DIRNAME)).catch(() => [])
	]);
	for (const name of reposEntries) if (name.startsWith("session-") && name.endsWith(".git")) ids.add(name.slice(8, -4));
	for (const name of wsEntries) if (name.startsWith("session-") && name.endsWith(".git")) ids.add(name.slice(8, -4));
	for (const name of backupEntries) if (name.startsWith("ws-session-")) ids.add(name.slice(11));
	else if (name.startsWith("session-")) ids.add(name.slice(8));
...
async function storedSessionUsage(root, sessionId) {
...
async function clearCache(root, scope, sessionIds) {
	...
	const repoFreed = (wantSession ? before.sessionBytes : 0) + (wantWorkspace ? before.workspaceBytes : 0);
	let removed = 0;
	let failed = 0;
	let backupsFreed = 0;
	/** Remove every child of `dir`, keeping `dir` itself. */
	const clearChildren = async (dir, keep) => {
		let entries;
		try {
		...
		for (const name of entries) {
			if (keep !== void 0 && keep(name)) continue;
	...
	await clearChildren(join(root, BACKUPS_DIRNAME), scope === "both" ? void 0 : (name) => scope === "workspace" ? !name.startsWith("ws-session-") : name.startsWith("ws-session-"));
	return {
		ok: failed === 0,
		freedBytes: repoFreed + backupsFreed,
		removed,
		failed
	};
}
...
function fileChangesSuffix(changes) {
	if (changes === void 0 || changes.length === 0) return "";
	const tuples = changes.map((change) => [change.status, change.path]);
	const compressed = deflateRawSync(Buffer$1.from(JSON.stringify(tuples), "utf8"));
	return `[F1:${compressed.toString("base64url")}]`;
}
/** Remove and decode the optional file manifest before parsing the base subject. */
function splitFileChanges(subject) {
	const match = FILES_MARKER.exec(subject);
	if (match === null) return { subject };
	const base$1 = subject.slice(0, match.index);
	try {
		const bytes = Buffer$1.from(match[1], "base64url");
		const parsed = JSON.parse(inflateRawSync(bytes).toString("utf8"));
		if (!Array.isArray(parsed)) return { subject: base$1 };
		const changes = [];
		for (const entry of parsed) {
			if (!Array.isArray(entry) || entry.length !== 2) return { subject: base$1 };
			const [status, path] = entry;
			if (status !== "A" && status !== "M" && status !== "D" || typeof path !== "string" || path.length === 0) return { subject: base$1 };
			changes.push({
				status,
				path
			});
		}
		return changes.length === 0 ? { subject: base$1 } : {
			subject: base$1,
			changes
		};
	} catch {
		return { subject: base$1 };
	}
}
/**
...
function buildSessionMessage(meta) {
	...
	if (meta.kind === "refine") {
		const turns = (meta.maskedTurns ?? []).filter((turn) => Number.isSafeInteger(turn) && turn >= 0).filter((turn, index, values) => values.indexOf(turn) === index).sort((a, b) => a - b);
		const baseMarker = meta.turn !== void 0 && Number.isSafeInteger(meta.turn) && meta.turn >= 0 ? `[BASE:${meta.turn}]` : "";
		const turnMarker = turns.length > 0 ? `[TURNS:${turns.join(",")}]` : "";
		const wsMarker = meta.ws !== void 0 && meta.ws.length > 0 ? `[${meta.ws}]` : "";
...
function parseMessage(subject) {
	...
	return split.changes === void 0 ? meta : {
		...meta,
...
function parseBracket(line) {
	...
	if (curate !== null) {
		const maskedTurns = curate[2] === void 0 ? [] : curate[2].split(",").map(Number).filter((turn$1) => Number.isSafeInteger(turn$1) && turn$1 >= 0);
		const baseTurn = curate[1] === void 0 ? void 0 : Number(curate[1]);
		return {
			kind: "refine",
			...baseTurn !== void 0 && Number.isSafeInteger(baseTurn) ? { turn: baseTurn } : {},
			...maskedTurns.length > 0 ? { maskedTurns } : {},
			...curate[3] !== void 0 && curate[3].length > 0 ? { ws: curate[3] } : {}
		};
	}
	const refine = /^\[REFINE\](?:\[([^\]]*)\])?$/.exec(line);
	if (refine !== null) return {
		kind: "refine",
		...refine[1] !== void 0 && refine[1].length > 0 ? { ws: refine[1] } : {}
	};
...
function compileExcludes(patterns) {
	...
	return (rel, isDir) => {
		...
		const invalid = normalized.length === 0 || normalized.startsWith("/") || normalized === "." || normalized.startsWith("./") || normalized.split("/").includes("..");
...
async function walkFiles(rootDir, matcher) {
	...
	const walk = async (absDir, relDir) => {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) await walk(join(absDir, name), rel);
...
function parseWorkspaceChanges(stdout) {
...
async function snapshotWorkspace(subprocess, root, sessionId, cwd, message) {
	...
	try {
		...
		let changes;
		if (parentTree === void 0) changes = files.map((file) => ({
			status: "A",
			path: file.rel
		}));
		else {
			const diff = await runGit(subprocess, [
				"git",
				`--git-dir=${repoDir}`,
				"diff-tree",
				"--no-commit-id",
...
async function takeSnapshot(subprocess, root, sessions, persistence, request) {
	...
	const snap = kind === "manual" ? `manual-${Date.now()}` : kind === "refine" ? `refine-${Date.now()}` : `turn-${derived}-${kind === "turn-start" ? "start" : "end"}-${seq ?? 0}`;
	const meta = {
		kind,
		...
		session: sessionId,
		snap,
		...kind === "refine" && request.baselineTurn !== void 0 ? { turn: request.baselineTurn } : {},
		...kind === "refine" && request.maskedTurns !== void 0 ? { maskedTurns: request.maskedTurns.slice().sort((a, b) => a - b) } : {},
	...
	const wsPromise = request.skipWorkspace !== true && cwd !== void 0 && cwd.length > 0 ? snapshotWorkspace(subprocess, root, sessionId, cwd, buildWorkspaceMessage(meta)) : Promise.resolve({ ok: true });
	...
	const workspaceChanges = ws.ok ? ws.changes : void 0;
	...
	if (baseBlob !== null && baseBlob.exitCode === 0) {
		if (firstLine(baseBlob.stdout) === blobSha && !workspaceChanged && !(kind === "turn-start" && !fromJump)) return {
			ok: true,
			unchanged: true,
			base: base$1,
			snap,
		...
		if (fromJump && base$1 !== void 0 && !workspaceChanged) {
	...
	const committed = await runGit(subprocess, argvCommitTree(sessionRepo, rootTreeSha, base$1), root, env, message);
	if (committed.exitCode !== 0) return {
		ok: false,
		reason: "commit-failed"
	};
...
*/
async function cleanupSuccessfulBackup(backup) {
	const retained = {};
	if (backup.session !== void 0) try {
		await rm(backup.session, { force: true });
	} catch {
		retained.session = backup.session;
	}
	if (backup.workspace !== void 0) try {
		await rm(backup.workspace, {
			recursive: true,
			force: true
		});
	} catch {
	...
	return retained.session === void 0 && retained.workspace === void 0 ? void 0 : retained;
}
/**
...
async function rewindSession(subprocess, root, sessions, persistence, agents, sessionId, commit, restoreWorkspace, agentPresets, workspaceOnly = false) {
	...
	if (backup.session !== void 0) cleanupTargets.session = backup.session;
	if (backup.workspace !== void 0 && workspaceRestored) cleanupTargets.workspace = backup.workspace;
	const retained = await cleanupSuccessfulBackup(cleanupTargets);
	const keep = { ...retained };
	if (backup.workspace !== void 0 && !workspaceRestored) keep.workspace = backup.workspace;
	if (keep.session === void 0 && keep.workspace === void 0) delete outcome.backup;
	else outcome.backup = keep;
	if (retained !== void 0) outcome.backupCleanupFailed = true;
	return outcome;
}
...
function replayEventInbox(events) {
	const state = {
		"next-turn": [],
		"next-step": []
	};
...
function mapEventSeqs(value, oldToNew) {
	...
	for (const source of value) {
		...
		if (next === void 0 || seen.has(next)) continue;
		seen.add(next);
		mapped.push(next);
	}
	return mapped;
...
*/
function curateSessionEvents(sourceEvents, masked) {
	const ordered = sourceEvents.slice().sort((a, b) => a.seq - b.seq);
	for (let index = 0; index < ordered.length; index += 1) if (ordered[index].seq !== index) throw new Error(`source event seq gap at index ${index} (read ${ordered[index].seq})`);
	...
	for (const event of ordered) {
		const ownTurn = typeof event.data.turn === "number" && Number.isSafeInteger(event.data.turn) ? event.data.turn : null;
	...
	const preliminary = ordered.filter((event) => {
		...
		if (event.surfaceOp !== void 0 && event.surfaceOp !== "append") return false;
		const turn = turnOf.get(event.seq);
		return turn === void 0 || !masked.has(turn);
	});
	...
	const kept = preliminary.filter((event) => {
		...
		const cited = messageSeqs.filter((seq) => typeof seq === "number");
	...
	const turnToDerived = new Map();
	for (let index = 0; index < retainedTurnOrder.length; index += 1) turnToDerived.set(retainedTurnOrder[index], index + 1);
	const oldToNew = new Map();
	for (let index = 0; index < kept.length; index += 1) oldToNew.set(kept[index].seq, index);
	const events = kept.map((source, index) => {
		...
		if (typeof sourceTurn === "number" && Number.isSafeInteger(sourceTurn)) {
			...
			if (derivedTurn !== void 0) event.data.turn = derivedTurn;
		}
		const mappedSources = mapEventSeqs(source.sourceEventSeqs, oldToNew);
		if (mappedSources !== void 0) if (mappedSources.length > 0 || event.type === "assistant/message") event.sourceEventSeqs = mappedSources;
		else delete event.sourceEventSeqs;
		const mappedMessages = mapEventSeqs(source.data.messageSeqs, oldToNew);
		if (mappedMessages !== void 0) event.data.messageSeqs = mappedMessages;
		return event;
	});
	let nextTime = 1;
...
function sourceSessionTitle(inspected) {
	let title = typeof inspected.meta.title === "string" ? inspected.meta.title.trim() : "";
	for (const event of inspected.events) if (event.type === "session/title" && typeof event.data.title === "string") {
...
*/
async function refineSession(subprocess, root, sessions, persistence, agents, sessionController, sourceSessionId, maskedTurns) {
	const source = sessions?.get(sourceSessionId);
	if (source === void 0 || source.header === void 0) return {
		ok: false,
		reason: "no-session"
	};
	if (agents?.get(sourceSessionId)?.status === "running") return {
		ok: false,
		reason: "session-running"
	};
	if (sessionController === void 0) return {
		ok: false,
		reason: "derived-session-unavailable"
	};
	...
	try {
		...
		return {
			ok: false,
			reason: "curation-failed",
			error: error instanceof Error ? error.message : String(error)
		};
	}
	if (curated.maskedTurns.length === 0) return {
		ok: false,
	...
	try {
		await sessionController.rename({
	...
	if (created.workspaceAttachmentFailure !== void 0) warnings.push(`Workspace attachment failed: ${created.workspaceAttachmentFailure.message}`);
	...
	let curationCommit;
	if (derivedSession === void 0) warnings.push("Derived Session was published but is not visible in the Session store yet");
	else {
	...
	return {
		...
		...curationCommit === void 0 ? {} : { curationCommit },
		workspaceAttached: created.workspaceAttached,
		...created.workspaceId === void 0 ? {} : { workspaceId: created.workspaceId },
		...created.workspaceAttachmentFailure === void 0 ? {} : { workspaceAttachmentFailure: created.workspaceAttachmentFailure },
...
async function timelineRows(subprocess, repoDir, _sessionId, root, _workspaceCwd, _includeFiles = true) {
	const repo = { gitDir: repoDir };
	...
	return async (req, res) => {
		try {
			...
			if (pathname === `${ROUTE_PREFIX}/timeline` && (method === "GET" || method === "POST")) {
				...
				const rows = await timelineRows(engine.subprocess, repoDir, sessionId$1, engine.root);
			...
			if (pathname === `${ROUTE_PREFIX}/rewind-status` && method === "GET") {
				...
				const status = engine.agents?.get(sessionId$1)?.status;
				json(res, 200, {
					ok: true,
			...
			if (pathname === `${ROUTE_PREFIX}/cache/session-usage` && method === "GET") {
				...
				const item = await storedSessionUsage(engine.root, sessionId$1);
				json(res, 200, {
					ok: true,
					session: item
				});
			...
			if (pathname === `${ROUTE_PREFIX}/cache/sessions` && method === "GET") {
				...
				const enriched = stored.map((item) => {
					...
					const title = s?.title ?? s?.header?.title ?? s?.header?.customTitle;
					return {
						...item,
			...
			if (pathname === `${ROUTE_PREFIX}/config`) {
				...
				const patch = {};
				if (typeof configArgs.gitignoreTemplate === "string") patch.gitignoreTemplate = configArgs.gitignoreTemplate;
				if (configArgs.cacheCapacityGb !== void 0) {
			...
			if (pathname === `${ROUTE_PREFIX}/cache/clear`) {
				...
				const clearArgs = clearBody !== null && typeof clearBody === "object" ? clearBody : {};
				...
				const sessionIds = Array.isArray(clearArgs.sessionIds) ? clearArgs.sessionIds.filter((id) => typeof id === "string" && id.length > 0) : void 0;
				const cleared = await clearCache(engine.root, scope, sessionIds);
				json(res, 200, {
					...cleared,
					usage: await cacheUsage(engine.root)
				});
				return;
			}
			let body;
			...
			if (pathname === `${ROUTE_PREFIX}/refine`) {
				const turns = Array.isArray(args.turns) ? args.turns.filter((turn) => typeof turn === "number" && Number.isFinite(turn) && Number.isSafeInteger(turn)) : [];
				if (turns.length === 0) {
					json(res, 400, {
						ok: false,
						reason: "bad-args"
				...
				const result = await gate.run(sessionId, () => refineSession(engine.subprocess, engine.root, engine.sessions, engine.persistence, engine.agents, engine.sessionController, sessionId, turns));
				json(res, 200, result);
				return;
			}
