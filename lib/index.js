...
*/
async function listStoredSessions(root) {
	const ids = new Set();
...
async function timelineRows(subprocess, repoDir, sessionId, root, workspaceCwd) {
	...
	if (res.exitCode !== 0) {
		...
		if (err.includes("does not have any commits") || err.includes("bad default revision") || err.includes("unknown revision") || res.stdout.trim().length === 0 && res.stderr.trim().length === 0) return [];
		return null;
	}
	const rows = [];
