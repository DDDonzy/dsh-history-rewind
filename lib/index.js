...
import { copyFile, cp, lstat, mkdir, open, readFile, readdir, rename, rm, stat, unlink, writeFile } from "node:fs/promises";
...
const HISTORY_ROOT_DIRNAME = ".dsh-history";
...
const WORKSPACE_DEFAULT_EXCLUDES = [
	".git",
	"node_modules",
	"dist",
	"build",
...
function workspaceRepoDir(root, cwd) {
...
function workspaceBackupDir(root, cwd) {
...
*/
async function readExcludes(root, cwd) {
	...
	const parsed = lines.map((line) => line.trim()).filter((line) => line.length > 0 && !line.startsWith("#"));
...
*/
async function ensureExcludes(root, cwd) {
	const repoDir = workspaceRepoDir(root, cwd);
	const excludePath = join(repoDir, "info", "exclude");
	if (existsSync(excludePath)) return;
	try {
		await mkdir(join(repoDir, "info"), { recursive: true });
		await writeFile(excludePath, `# dsh-history workspace excludes (one basename or glob per line; '#' comments).\n${WORKSPACE_DEFAULT_EXCLUDES.join("\n")}\n`, "utf-8");
	} catch {}
}
...
*/
async function snapshotWorkspace(subprocess, root, cwd, message) {
...
*/
async function backupWorkspace(root, cwd) {
...
*/
function extractMessagePreviews(bytes) {
	let frames;
	try {
	...
	for (let f = frames.length - 1; f >= 0; f -= 1) {
		...
		for (let i = lines.length - 1; i >= 0; i -= 1) {
			...
			if (assistant === void 0 && parsed.type === "assistant/message" && data !== void 0) {
				const message = data.message;
				const content = message?.content ?? data.content;
				const value = textOf(content);
				if (value.length > 0) assistant = value;
			} else if (user === void 0 && parsed.type === "user/message" && data !== void 0) {
			...
			if (user !== void 0 && assistant !== void 0) return {
				user,
				assistant
			};
		}
...
*/
async function captureSessionArtifact(subprocess, root, sessions, persistence, session, kind = "turn-end") {
...
async function takeSnapshot(subprocess, root, sessions, persistence, request) {
	...
	const wsPromise = cwd !== void 0 && cwd.length > 0 ? snapshotWorkspace(subprocess, root, cwd, buildWorkspaceMessage(meta)) : Promise.resolve({ ok: true });
...
async function rewindSession(subprocess, root, sessions, persistence, agents, sessionId, commit, restoreWorkspace, agentPresets, workspaceOnly = false) {
	...
	if (workspaceOnly) {
		...
		const wsRepo = { gitDir: workspaceRepoDir(root, cwdWs) };
		const wsVerify = await runGit(subprocess, argvRevParseCommit(wsRepo, wsCommit$1), root, env);
		if (wsVerify.exitCode !== 0 || firstLine(wsVerify.stdout).length === 0) return {
			ok: false,
			reason: "no-workspace-snapshot",
			target
		};
		...
		const restored = await materializeTree(subprocess, wsRepo.gitDir, wsCommit$1, cwdWs);
		return {
			ok: restored !== null,
			target,
			workspaceOnly: true,
	...
	if (restoreWorkspace && cwd !== void 0 && cwd.length > 0) {
		...
		if (wsCommit === null) noWorkspaceSnapshot = true;
			...
			const wsRepo = { gitDir: workspaceRepoDir(root, cwd) };
...
async function timelineRows(subprocess, repoDir, root, workspaceCwd) {
	const repo = { gitDir: repoDir };
	...
	if (workspaceCwd !== void 0 && workspaceCwd !== null && workspaceCwd.length > 0) {
		const wsRepo = { gitDir: workspaceRepoDir(root, workspaceCwd) };
		const log = await runGit(subprocess, [
			"git",
			`--git-dir=${wsRepo.gitDir}`,
			"log",
			"--root",
			"--name-only",
			"--format=commit %H",
		...
		if (log.exitCode === 0) {
			...
			for (const row of rows) {
				...
				if (files !== void 0 && files.length > 0) row.files = files;
			}
		}
	}
	return rows;
...
async function purgeSession(subprocess, root, sessionId, cwd, keepBackups = 3) {
	...
	const wsDeleted = cwd !== void 0 && cwd.length > 0 ? await rotateBackups(workspaceBackupDir(root, cwd), keepBackups) : 0;
...
function apply(ctx) {
	...
	const captureOrdered = (session, kind) => {
		const previous = captureChains.get(session.id) ?? Promise.resolve();
		const next = previous.then(() => captureSessionArtifact(subprocess, root, sessions, persistence, session, kind), () => captureSessionArtifact(subprocess, root, sessions, persistence, session, kind));
