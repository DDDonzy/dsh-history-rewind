...
*/
async function readExcludes(root, sessionId) {
...
*/
async function ensureExcludes(root, sessionId) {
	const repoDir = workspaceRepoDir(root, sessionId);
	const excludePath = join(repoDir, "info", "exclude");
...
async function rewindSession(subprocess, root, sessions, persistence, agents, sessionId, commit, restoreWorkspace, agentPresets, workspaceOnly = false) {
	...
	if (workspaceOnly) {
		...
		const restored = await materializeTree(subprocess, wsGit, wsCommit$1, cwdWs);
		return {
			ok: restored !== null,
			target,
			workspaceOnly: true,
